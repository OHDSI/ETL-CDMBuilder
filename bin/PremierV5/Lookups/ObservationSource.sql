...
FROM CTE_VOCAB_MAP
WHERE SOURCE_VOCABULARY_ID IN ('ICD9CM', 'CPT4', 'HCPCS', 'MDC')
AND TARGET_VOCABULARY_ID IN  ('ICD9CM', 'CPT4', 'HCPCS', 'MDC') and TARGET_DOMAIN_ID='Observation'
union
SELECT distinct SOURCE_CODE, TARGET_CONCEPT_ID
FROM CTE_VOCAB_MAP
WHERE SOURCE_VOCABULARY_ID IN ( 'DRG')
AND TARGET_VOCABULARY_ID IN  ('DRG') and TARGET_DOMAIN_ID='Observation'
