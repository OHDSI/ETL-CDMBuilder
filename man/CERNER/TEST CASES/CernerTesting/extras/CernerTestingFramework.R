...
add_U_version <- function(version_id, version_date) {
  ...
  if (exists('testNewAdded', where = frameworkContext) && get('testNewAdded', envir = frameworkContext))
  {
...
add_hf_d_patient_type <- function(patient_type_id, patient_type_desc) {
  ...
  if (exists('testNewAdded', envir = frameworkContext) && get('testNewAdded', envir = frameworkContext))
  {
    frameworkContext$testNewAdded <- FALSE
    id <- frameworkContext$testId
    description <- frameworkContext$testDescription
...
expect_location <- function(location_id, address_1, address_2, city, state, zip, county, location_source_value) {
 ...
 statement <- paste0(statement, paste0(whereClauses, collapse=" AND "));
  ...
  if (exists('testNewExpected', where = frameworkContext) && get('testNewExpected', envir = frameworkContext))
  {
...
expect_observation_period <- function(observation_period_id, person_id, observation_period_start_date, observation_period_start_datetime, observation_period_end_date, observation_period_end_datetime, period_type_concept_id) {
  ...
  if (!missing(observation_period_start_datetime)) {
    if (is.null(observation_period_start_datetime)) {
      whereClauses <- c(whereClauses, "observation_period_start_datetime IS NULL")
    } else if (is(observation_period_start_datetime, "subQuery")){
      whereClauses <- c(whereClauses, paste0("observation_period_start_datetime = (", as.character(observation_period_start_datetime), ")"))
    } else {
      whereClauses <- c(whereClauses, paste0("observation_period_start_datetime = '", observation_period_start_datetime,"'"))
    }
  }
  ...
  if (!missing(observation_period_end_datetime)) {
    if (is.null(observation_period_end_datetime)) {
      whereClauses <- c(whereClauses, "observation_period_end_datetime IS NULL")
    } else if (is(observation_period_end_datetime, "subQuery")){
      whereClauses <- c(whereClauses, paste0("observation_period_end_datetime = (", as.character(observation_period_end_datetime), ")"))
    } else {
      whereClauses <- c(whereClauses, paste0("observation_period_end_datetime = '", observation_period_end_datetime,"'"))
    }
  }
