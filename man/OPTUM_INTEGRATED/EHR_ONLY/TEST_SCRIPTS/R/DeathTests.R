createDeathTests <- function () {
  ...
  tables_to_check <- c("diagnosis", "encounter", "immunization", "insurance", "labs", "medication_administrations",
                       "microbiology", "nlp_biomarker", "nlp_drug_rationale", "nlp_measurements", "nlp_sds", "nlp_sds_family",
                       "observations", "patient_reported_meds", "prescriptions_written", "procedure", "visit")
...
createDeathDataForTables <- function(tables_to_check, death_date_desc, death_date, end_date, expect_function) {
  for(tableName in tables_to_check) {
    ...
    if (tableName =="nlp_biomarker")
      add_nlp_biomarker(ptid=patient$ptid, note_date=end_date)
    if (tableName =="nlp_drug_rationale")
      add_nlp_biomarker(ptid=patient$ptid, note_date=end_date)
    if (tableName =="nlp_measurements")
      add_nlp_measurements(ptid=patient$ptid, encid=enc$encid, note_date=end_date)
    ...
    if (tableName =="patient_reported_meds")
      add_patient_reported_meds(ptid=patient$ptid, reported_date=end_date)
