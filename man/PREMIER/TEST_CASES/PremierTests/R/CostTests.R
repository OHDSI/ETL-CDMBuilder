createCostTests <- function () {
  ...
  visit <- createVisit()
  declareTest(description="Cost record from drug_exposure", source_pid=patient$medrec_key, cdm_pid = patient$person_id);
  ...
  expect_cost(cost_domain_id            = "Drug",
              total_charge              = 500.50,
              total_cost                = 300.75,
              revenue_code_concept_id   = 38003138,
              revenue_code_source_value = "PHARMACY / PHARMACY");


  patient <- createPatient()
  visit <- createVisit()
  declareTest(description="Cost record from procedure_occurrence", source_pid=patient$medrec_key, cdm_pid = patient$person_id);
  add_pat(medrec_key = patient$medrec_key,
          pat_key    = visit$pat_key);
  add_patbill(pat_key       = visit$pat_key,
  ...
  patient <- createPatient()
  visit <- createVisit()
  declareTest(description="Cost record from device_exposure", source_pid=patient$medrec_key, cdm_pid = patient$person_id);
  add_pat(medrec_key = patient$medrec_key,
          pat_key    = visit$pat_key);
  add_patbill(pat_key       = visit$pat_key,
  ...
  expect_cost(cost_domain_id            = "Device",
              total_charge              = 499.99,
              total_cost                = 99.99,
              revenue_code_concept_id   = 38003154,
              revenue_code_source_value = "SUPPLY / CENTRAL SUPPLY");


  patient <- createPatient()
  visit <- createVisit()
  declareTest(description="Cost record from observation", source_pid=patient$medrec_key, cdm_pid = patient$person_id);
  add_pat(medrec_key = patient$medrec_key,
          pat_key    = visit$pat_key);
  add_patbill(pat_key       = visit$pat_key,
  ...
  expect_observation(person_id              = patient$person_id,
                     observation_concept_id = 4033670);
  ...
  patient <- createPatient()
  visit <- createVisit()
  declareTest(description="Cost record from measurement", source_pid=patient$medrec_key, cdm_pid = patient$person_id);
  add_pat(medrec_key = patient$medrec_key,
          pat_key    = visit$pat_key);
  add_patbill(pat_key       = visit$pat_key,
  ...
  expect_measurement(person_id            = patient$person_id,
                     measurement_concept_id = 4242839);
