createPayerPlanPeriodTests <- function () {
  ...
  declareTest(description = "Creates two payer_plan_period records, one payer", source_pid=patient$medrec_key, cdm_pid=patient$person_id);
  visit1 <- createVisit();
  visit2 <- createVisit();
  visit3 <- createVisit();
  visit4 <- createVisit();
  ...
  patient <- createPatient()
  declareTest(description = "Creates two payer_plan_period records, two payers", source_pid=patient$medrec_key, cdm_pid=patient$person_id);
  visit1 <- createVisit();
  visit2 <- createVisit();
  visit3 <- createVisit();
  visit4 <- createVisit();
  ...
  visit4 <- createVisit()
  visit5 <- createVisit()
  declareTest(description = "Creates bad record with payer_plan_period_end_date < payer_plan_period_start_date", source_pid=patient$medrec_key, cdm_pid = patient$person_id);
  ...
  # visit4 <- createVisit()
  # visit5 <- createVisit()
  # declareTest(description = "Creates good records where payer_plan_period_end_date > payer_plan_period_start_date", source_pid=patient$medrec_key, cdm_pid = patient$person_id);
