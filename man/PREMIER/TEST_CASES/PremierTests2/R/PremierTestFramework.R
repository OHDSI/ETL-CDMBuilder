initFramework <- function() {
  ...
  defaults <- list()
  assign('admsrc', defaults, envir = frameworkContext$defaultValues)
  ...
  defaults <- list()
  defaults$hosp_chg_desc <- 'NAME NOT FOUND'
  ...
  defaults <- list()
  defaults$adm_mon <- '2013410'
  defaults$adm_date <- '2013-10-01'
  defaults$disc_mon <- '2013410'
  defaults$disc_date <- '2013-10-01'
  ...
  defaults$hosp_chg_id <- '422222825'
  defaults$serv_day <- '1'
  ...
  assign('readmit', defaults, envir = frameworkContext$defaultValues)
}
...
set_defaults_admsrc <- function(adm_source, adm_src_desc) {
  defaults <- get('admsrc', envir = frameworkContext$defaultValues)
...
set_defaults_patbill <- function(pat_key, std_chg_code, hosp_chg_id, serv_day, hosp_qty, std_qty, bill_charges, bill_cost, bill_var_cost, bill_fix_cost, period) {
...
set_defaults_readmit <- function(medrec_key, pat_key, i_o_ind, in_extract, disc_mon, disc_mon_seq, days_from_prior, calc_los, gender, race, pat_type, point_of_origin, disc_status, adm_type, ms_drg, age, icd_version, prim_icd_diag_code, prim_icd_proc_code, pat_cost, pat_charges) {
  defaults <- get('readmit', envir = frameworkContext$defaultValues)
  if (!missing(medrec_key)) {
    defaults$medrec_key <- medrec_key
  }
  if (!missing(pat_key)) {
...
add_admsrc <- function(adm_source, adm_src_desc) {
  ...
  if (!is.null(adm_source)) {
    fields <- c(fields, "adm_source")
    values <- c(values, if (is.null(adm_source)) "NULL" else if (is(adm_source, "subQuery")) paste0("(", as.character(adm_source), ")") else paste0("'", as.character(adm_source), "'"))
  }
  ...
  if (!is.null(adm_src_desc)) {
    fields <- c(fields, "adm_src_desc")
    values <- c(values, if (is.null(adm_src_desc)) "NULL" else if (is(adm_src_desc, "subQuery")) paste0("(", as.character(adm_src_desc), ")") else paste0("'", as.character(adm_src_desc), "'"))
  }

  inserts <- list(testId = frameworkContext$testId, testDescription = frameworkContext$testDescription, table = "admsrc", fields = fields, values = values)
  frameworkContext$inserts[[length(frameworkContext$inserts) + 1]] <- inserts
  invisible(NULL)
}
...
add_icdcode <- function(icd_version, icd_code, icd_desc, ccs_cat_level1_code, ccs_cat_level1_desc, ccs_cat_level2_code, ccs_cat_level2_desc, icd_diag_proc, icd_diag_proc_desc) {
  ...
  if (!is.null(ccs_cat_level1_code)) {
  ...
  if (!is.null(ccs_cat_level1_desc)) {
  ...
  if (!is.null(ccs_cat_level2_code)) {
  ...
  if (!is.null(ccs_cat_level2_desc)) {
...
add_pat <- function(pat_key, adm_mon, adm_date, disc_mon, disc_date, prov_id, i_o_ind, pat_type, apr_sev, apr_mort, adm_source, point_of_origin, adm_type, disc_status, mart_status, age, gender, race, admphy_spec, adm_phy, attphy_spec, att_phy, std_payor, los, proj_wgt, pat_charges, pat_cost, pat_fix_cost, pat_var_cost, disc_mon_seq, period, medrec_key, apr_drg, ms_drg, ms_drg_mdc, hispanic_ind) {
  ...
  if (!is.null(adm_mon)) {
    fields <- c(fields, "adm_mon")
    values <- c(values, if (is.null(adm_mon)) "NULL" else if (is(adm_mon, "subQuery")) paste0("(", as.character(adm_mon), ")") else paste0("'", as.character(adm_mon), "'"))
  }
  ...
  if (!is.null(proj_wgt)) {
    fields <- c(fields, "proj_wgt")
    values <- c(values, if (is.null(proj_wgt)) "NULL" else if (is(proj_wgt, "subQuery")) paste0("(", as.character(proj_wgt), ")") else paste0("'", as.character(proj_wgt), "'"))
  }
  ...
  if (!is.null(disc_mon_seq)) {
    fields <- c(fields, "disc_mon_seq")
    values <- c(values, if (is.null(disc_mon_seq)) "NULL" else if (is(disc_mon_seq, "subQuery")) paste0("(", as.character(disc_mon_seq), ")") else paste0("'", as.character(disc_mon_seq), "'"))
  }
  ...
  if (!is.null(period)) {
    fields <- c(fields, "period")
    values <- c(values, if (is.null(period)) "NULL" else if (is(period, "subQuery")) paste0("(", as.character(period), ")") else paste0("'", as.character(period), "'"))
  }
...
add_patbill <- function(pat_key, std_chg_code, hosp_chg_id, serv_day, hosp_qty, std_qty, bill_charges, bill_cost, bill_var_cost, bill_fix_cost, period) {
  defaults <- get('patbill', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(pat_key)) {
  ...
  if (!is.null(serv_day)) {
    fields <- c(fields, "serv_day")
    values <- c(values, if (is.null(serv_day)) "NULL" else if (is(serv_day, "subQuery")) paste0("(", as.character(serv_day), ")") else paste0("'", as.character(serv_day), "'"))
  }
...
add_paticd_diag <- function(pat_key, icd_version, icd_source, icd_code, icd_pri_sec, icd_poa, period) {
  ...
  if (!is.null(icd_source)) {
    fields <- c(fields, "icd_source")
    values <- c(values, if (is.null(icd_source)) "NULL" else if (is(icd_source, "subQuery")) paste0("(", as.character(icd_source), ")") else paste0("'", as.character(icd_source), "'"))
  }
...
add_paticd_proc <- function(pat_key, icd_version, icd_source, icd_code, icd_pri_sec, proc_day, proc_phy, procphy_spec, period) {
  ...
  if (!is.null(proc_day)) {
    fields <- c(fields, "proc_day")
    values <- c(values, if (is.null(proc_day)) "NULL" else if (is(proc_day, "subQuery")) paste0("(", as.character(proc_day), ")") else paste0("'", as.character(proc_day), "'"))
  }
...
add_readmit <- function(medrec_key, pat_key, i_o_ind, in_extract, disc_mon, disc_mon_seq, days_from_prior, calc_los, gender, race, pat_type, point_of_origin, disc_status, adm_type, ms_drg, age, icd_version, prim_icd_diag_code, prim_icd_proc_code, pat_cost, pat_charges) {
  ...
  if (!is.null(in_extract)) {
    fields <- c(fields, "in_extract")
    values <- c(values, if (is.null(in_extract)) "NULL" else if (is(in_extract, "subQuery")) paste0("(", as.character(in_extract), ")") else paste0("'", as.character(in_extract), "'"))
  }
  ...
  if (!is.null(days_from_prior)) {
    fields <- c(fields, "days_from_prior")
    values <- c(values, if (is.null(days_from_prior)) "NULL" else if (is(days_from_prior, "subQuery")) paste0("(", as.character(days_from_prior), ")") else paste0("'", as.character(days_from_prior), "'"))
  }
  ...
  if (!is.null(calc_los)) {
    fields <- c(fields, "calc_los")
    values <- c(values, if (is.null(calc_los)) "NULL" else if (is(calc_los, "subQuery")) paste0("(", as.character(calc_los), ")") else paste0("'", as.character(calc_los), "'"))
  }
  ...
  if (!is.null(prim_icd_diag_code)) {
    fields <- c(fields, "prim_icd_diag_code")
    values <- c(values, if (is.null(prim_icd_diag_code)) "NULL" else if (is(prim_icd_diag_code, "subQuery")) paste0("(", as.character(prim_icd_diag_code), ")") else paste0("'", as.character(prim_icd_diag_code), "'"))
  }
  ...
  if (!is.null(prim_icd_proc_code)) {
    fields <- c(fields, "prim_icd_proc_code")
    values <- c(values, if (is.null(prim_icd_proc_code)) "NULL" else if (is(prim_icd_proc_code, "subQuery")) paste0("(", as.character(prim_icd_proc_code), ")") else paste0("'", as.character(prim_icd_proc_code), "'"))
  }
  ...
  inserts <- list(testId = frameworkContext$testId, testDescription = frameworkContext$testDescription, table = "readmit", fields = fields, values = values)
  frameworkContext$inserts[[length(frameworkContext$inserts) + 1]] <- inserts
  invisible(NULL)
}
...
generateInsertSql <- function(databaseSchema = NULL) {
  ...
  insertSql <- c(insertSql, "TRUNCATE TABLE @cdm_database_schema.admsrc;")
  ...
  insertSql <- c(insertSql, "TRUNCATE TABLE @cdm_database_schema.readmit;")
