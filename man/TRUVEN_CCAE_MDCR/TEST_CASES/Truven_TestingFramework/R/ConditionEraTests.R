createConditionEraTests <- function () {
  ...
  declareTest("Patient condition_concept_id of 0, no condition_era record created", source_pid = patient$enrolid, cdm_pid = patient$person_id)
  ...
  patient <- createPatient()
  encounter <- createEncounter()  
  declareTest("Two records with same condition_concept_id are less than 30 days apart, one condition_era record created", source_pid = patient$enrolid, cdm_pid = patient$person_id)
  add_enrollment_detail(enrolid=patient$enrolid, dtend = '2012-12-31', dtstart = '2012-01-01')
  ...
  patient <- createPatient()
  encounter <- createEncounter()  
  declareTest("Two records with same condition_concept_id are more than 30 days apart, two condition_era records created", source_pid = patient$enrolid, cdm_pid = patient$person_id)
