...
FROM {sc}.MEMBER_DETAIL f
	JOIN (		/*find person's last record - handles tie breakers*/
		...
		FROM {sc}.MEMBER_DETAIL m2
			JOIN (
				SELECT m.patid, MAX(m.eligeff) AS eligeff
				FROM {sc}.MEMBER_DETAIL m
				GROUP BY m.patid
			) m1
	...
	LEFT OUTER JOIN (
		SELECT m.PATID, MIN(DATEPART(year,m.ELIGEFF)) AS MIN_ENROLL_YEAR
		FROM {sc}.MEMBER_DETAIL m
		GROUP BY m.PATID
	) min_year
		ON min_year.PATID = f.PATID
WHERE f.YRDOB BETWEEN 1900 AND DATEPART(year,GETDATE())
