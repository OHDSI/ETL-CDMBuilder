...
namespace org.ohdsi.cdm.builders.premier
{
   ...
   public class PremierPersonBuilder : PersonBuilder
   {
      ...
      public PremierPersonBuilder(ChunkBuilder chunkBuilder)
         : base(chunkBuilder)
      {
      }
      ...
      public override void Build()
      {
         ...
         AddToChunk(person, death,
            ...
            procedureCosts,
            observations,
            visitOccurrences.Values.ToArray(), new Cohort[0]);
      }
         ...
         ObservationPeriod[] observationPeriods)
      {
         ...
         foreach (
            var vo in
               visitOccurrences.OrderBy(vo => vo.EndDate).ThenBy(vo => int.Parse(vo.AdditionalFields["disc_mon_seq"])))
         {
            var maxServDay = 0;
         ...
         Dictionary<long, VisitOccurrence> visitOccurrences, ObservationPeriod[] observationPeriods)
      {
         ...
         foreach (var drugExposure in drugExposures)
         {
            ...
            if (drugExposure.TypeConceptId == 38000180)
            {
               // Premier captures the day of administration in the SERV_DAY field thus, 
               // the DRUG_EXPOSURE_START_DATE is determined from the VISIT_START_DATE from VISIT_OCCURRENCE and PATBILL.SERV_DAY 
               // or unless the start date is greater than the end of the month, then it’s truncated to the end of month. 
               int servDay;
               int.TryParse(drugExposure.AdditionalFields["serv_day"], out servDay);

               if (servDay == 0 || servDay == 1)
               {
                  drugExposure.StartDate = visitOccurrence.StartDate;
               }
               else if (servDay != 1 && visitOccurrence.StartDate.AddDays(servDay - 1) < visitOccurrence.EndDate)
               {
                  drugExposure.StartDate = visitOccurrence.StartDate.AddDays(servDay - 1);
               }
               else
               {
      ...
      public override IEnumerable<ProcedureOccurrence> BuildProcedureOccurrences(
         ...
         ObservationPeriod[] observationPeriods)
      {
         var uniqueEntities = new HashSet<ProcedureOccurrence>();
         foreach (var procedureOccurrence in procedureOccurrences)
         {
            ...
            {
               int servDay;
               int.TryParse(procedureOccurrence.AdditionalFields["serv_day"], out servDay);

               if (servDay == 0 || servDay == 1)
               {
                  procedureOccurrence.StartDate = visitOccurrence.StartDate;
               }
               else if (servDay != 1 && visitOccurrence.StartDate.AddDays(servDay - 1) < visitOccurrence.EndDate)
               {
                  procedureOccurrence.StartDate = visitOccurrence.StartDate.AddDays(servDay - 1);
               }
               else
               {
            ...
            if (procedureOccurrence.ProcedureCosts != null && procedureOccurrence.ProcedureCosts.Count > 0)
            {
               foreach (var procedureCost in procedureOccurrence.ProcedureCosts)
               {
                  ...
                  var result = chunkBuilder.Vocabulary.Lookup(procedureCost.DiseaseClassSourceValue,
         ...
         Dictionary<long, VisitOccurrence> visitOccurrences)
      {
         ...
         {
            payerPlanPeriod.Id = chunkBuilder.Chunk.KeyMasterOffset.PayerPlanPeriodId;
         ...
         Dictionary<long, VisitOccurrence> visitOccurrences, ObservationPeriod[] observationPeriods)
      {
         ...
         {
            if (string.IsNullOrEmpty(observation.ValueAsString)) continue;
