   ...
   public class SeerPersonBuilder : PersonBuilder
   {
      ...
      private void AddDeath(Person record, Death death)
      {
         ...
         switch (record.AdditionalFields["name"].ToUpper())
         {
            ...
            case "SER_DODM":
               if (death.EndDate.HasValue && int.TryParse(record.AdditionalFields["value"], out month) && month > 0 &&
                   month <= 12)
               {
      ...
      public static void AddPeriods(ICollection<PayerPlanPeriod> preiods, string name, string value, string variableName)
      {
         ...
         foreach (var grouped in periods.GroupBy(p => p.SourceValue))
         {
            ...
            foreach (var p in grouped.OrderBy(p => p.StartDate))
            {
               ...
               else
               {
                  ...
                  {
                     if (p.PlanSourceValue != prev.PlanSourceValue || p.StartDate.Subtract(prev.EndDate.Value).Days > 1)
                     {
                        result.Add(prev);
                        prev = p;
                     }
      ...
      {
         var medpar = visitOccurrences.Where(vo => vo.AdditionalFields["source"] == "medpar" && vo.ConceptId != 42898160 && vo.EndDate.HasValue).ToList();
         var SNFvisits = visitOccurrences.Where(vo => vo.AdditionalFields["source"] == "medpar" && vo.ConceptId == 42898160 && vo.EndDate.HasValue).ToList();
         ...
         foreach (var vo in visitOccurrences)
         {
            ...
            var withinMedpar =
               medpar.Where(
                  v => vo.StartDate.Between(v.StartDate, v.EndDate.Value) || vo.EndDate.Value.Between(v.StartDate, v.EndDate.Value)).ToList();
            ...
            if (vo.AdditionalFields["source"] == "outsaf")
            {
               ...
               else if (withinMedpar.Any(m => vo.StartDate.Between(m.StartDate, m.EndDate.Value) && vo.EndDate.Value > m.EndDate.Value))
               {
                  outsaf.Add(vo);
               }
               else if (withinMedpar.Any(m => vo.EndDate.Value.Between(m.StartDate, m.EndDate.Value) && vo.StartDate < m.StartDate))
               {
                  outsaf.Add(vo);
               }
            ...
            else
            {
               ...
               if (withinMedpar.Count > 0)
               {
                  ...
                  {
                     if (vo.AdditionalFields["suitable_for_medpar_n"] == "1")//and the NCH or DME variable PLCSRVC must be in (4,31) 
                        continue; //rolled into a MEDPAR visit
                  }
                  else//If the MEDPAR visit has SNFIND other than ‘N’ 
                  {
                     if (vo.AdditionalFields["suitable_for_medpar_others"] == "1")//the NCH or DME variable PLCSRVC must be in (3,21,51,52,55,56,61,65) to be rolled into the MEDPAR 
                        continue; //rolled into a MEDPAR visit
                  }
               }
      ...
      private static IEnumerable<VisitOccurrence> ClenupDmeAndNch(IEnumerable<VisitOccurrence> visits, VisitOccurrence[] outsafER, VisitOccurrence[] outsafOutpatient)
      {
      ...
      protected void SetVisitOccurrenceId<T>(IEnumerable<T> inputRecords, VisitOccurrence[] visitOccurrences) where T : class, IEntity
      {
         foreach (var entity in inputRecords)
         {
            var visitOccurrence =
               visitOccurrences.FirstOrDefault(vo => vo.EndDate.HasValue && entity.StartDate.Between(vo.StartDate, vo.EndDate.Value));
         ...
         ObservationPeriod[] observationPeriods)
      {
         ...
         foreach (var po in base.BuildProcedureOccurrences(procedureOccurrences, visitOccurrences, observationPeriods))
         {
         ...
         ObservationPeriod[] observationPeriods)
      {
         ...
         foreach (var co in base.BuildConditionOccurrences(conditionOccurrences.ToArray(), visitOccurrences, observationPeriods))
         {
            cond.Add(co);
         }
      ...
      public override Death BuildDeath(Death[] death, Dictionary<long, VisitOccurrence> visitOccurrences, ObservationPeriod[] observationPeriods)
      {
         ...
         if (death.Any())
         {
            ...
            if (secondary != null && observationPeriods.Any(p => secondary.StartDate.Between(p.StartDate, p.EndDate.Value)))
               return secondary;
         }
         return null;
      }
      ...
      public override void Build(Dictionary<string, long> providers)
      {
         ...
         if (observationPeriods.Any() && observationPeriods.Any(period => person.YearOfBirth - period.StartDate.Year >= 2)) return;
         ...
         {
            if (visitOccurrence.StartDate.Year < person.YearOfBirth || visitOccurrence.EndDate.Value.Year < person.YearOfBirth) continue;
            if (visitOccurrence.StartDate.Year == person.YearOfBirth && visitOccurrence.StartDate.Month < person.MonthOfBirth) continue;
            if (visitOccurrence.EndDate.Value.Year == person.YearOfBirth && visitOccurrence.EndDate.Value.Month < person.MonthOfBirth) continue;
         ...
         var drugExposures =
            CleanUp(BuildDrugExposures(drugExposuresRaw.ToArray(), visitOccurrences, observationPeriods), death).ToArray();
         var procedureOccurrences =
            CleanUp(BuildProcedureOccurrences(procedureOccurrencesRaw.ToArray(), visitOccurrences, observationPeriods), death).ToArray();
         var observations = CleanUp(BuildObservations(observationsRaw.ToArray(), visitOccurrences, observationPeriods), death).ToArray();
         var measurements = CleanUp(BuildMeasurement(measurementsRaw.ToArray(), visitOccurrences, observationPeriods), death).ToArray();
         var deviceExposures =
            CleanUp(BuildDeviceExposure(deviceExposureRaw.ToArray(), visitOccurrences, observationPeriods), death).ToArray();
      ...
      public override void AddToChunk(string domain, IEnumerable<IEntity> entities)
      {
         foreach (var entity in entities)
         {
            ...
            switch (entityDomain)
            {
               ...
               case "Device":
                  deviceExposure.Add(entity as DeviceExposure ??
