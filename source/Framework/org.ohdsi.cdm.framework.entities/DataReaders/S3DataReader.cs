...
namespace org.ohdsi.cdm.framework.entities.DataReaders
{
   ...
   public class S3DataReaderLookup : IDataReader
   {
      ...
      public S3DataReaderLookup(string bucket, string fileKey, string awsAccessKeyId,
         string awsSecretAccessKey, Dictionary<string, int> fieldHeaders, Action<string> log)
      {
         this.bucket = bucket;
      ...
      private IEnumerator<string[]> GetEnumerator()
      {
         ...
         foreach (var line in lines.GetConsumingEnumerable())
         {
            if (!string.IsNullOrEmpty(line))
            {
               ...
               {
                  log("Split error, file name - " + fileKey);
